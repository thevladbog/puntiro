...
The fast `foundation:check` enforces reviewable repository-file boundaries, not friend-metadata origin or the complete imported MSBuild graph. The mandatory `check:dotnet` evaluates every project's effective `ProjectReference` items, including arbitrary and inherited imports, and requires the exact approved acyclic graph before it builds anything. It then performs locked restores and builds the complete solution in an empty per-run artifacts root, followed by the BCL-only verifier and every protected project separately under unique `--artifacts-path` roots.
