...
export const shipmentTaskCardDocumentation = defineDocumentation({
  ru: {
    ...
    content: ['Передавайте полный номер; не сокращайте его многоточием.', 'Время форматируется через Intl.DateTimeFormat для locale провайдера в UTC, чтобы examples и проверки были детерминированными.'],
    accessibility: ['Корень — нативная доступная button с полным номером в видимом тексте и accessible name.', 'Статус всегда содержит иконку и текст через StatusBadge.', 'Touch mode сохраняет цель не менее 64 px, standard mode — не менее 44 px.'],
  ...
  en: {
    ...
    accessibility: ['The root is a native accessible button with the full number in visible text and its accessible name.', 'Each status has text and an icon through StatusBadge.', 'Touch mode retains a 64 px target; standard mode retains 44 px.'],
